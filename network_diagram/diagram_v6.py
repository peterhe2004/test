...
# Add edges with attributes (such as SPort and DPort)
print("\nAdding edges to the graph:")
for index, row in edge_data.iterrows():
    if pd.notna(row['Target']):  # Ensure the 'Target' is not NaN
...
edge_labels = {(row['Source'], row['Target'], index): f"{row['SPort']} <---> {row['DPort']}"
               for index, row in edge_data.iterrows() if pd.notna(row['Target'])}

# Draw the edge labels on the graph
nx.draw_networkx_edge_labels(G, pos, edge_labels=edge_labels)

# Show the plot
plt.savefig('show_neighbor_topo6.png')
plt.show(block=True)